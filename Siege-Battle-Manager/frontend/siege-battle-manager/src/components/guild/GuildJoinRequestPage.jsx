...
export default function GuildJoinRequestPage({ me }) {
  ...
  async function loadPendingRequest() {
    try {
      ...
      setError(e.message || "가입 요청 상태를 불러오지 못했습니다.");
    }
  }
  ...
  async function handleSubmit(event) {
    ...
    if (pendingRequest) {
      setError("이미 처리 대기 중인 가입 요청이 있습니다. 철회 후 다시 요청해주세요.");
      return;
    }
    ...
    try {
      setLoading(true);
      await requestExistingAccountJoin(guildName.trim());
      await loadPendingRequest();
      setNotice("가입 요청을 보냈습니다. 길드장 또는 부길드장의 승인을 기다려주세요.");
      setGuildName("");
    } catch (e) {
      setError(e.message || "가입 요청을 보내지 못했습니다.");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleCancel() {
    const ok = window.confirm("가입 요청을 철회할까요?");
    if (!ok) return;
    ...
    try {
      ...
      setNotice("가입 요청을 철회했습니다.");
    } catch (e) {
      setError(e.message || "가입 요청을 철회하지 못했습니다.");
    } finally {
      setLoading(false);
    }
  }
        ...
        <section className="w-full rounded-2xl border border-[#7a5525] bg-[#2a160d] p-6 shadow-sm">
          <div className="flex flex-col gap-3 border-b border-[#51341e] pb-5 md:flex-row md:items-start md:justify-between">
            <div>
              ...
              <h1 className="mt-1 text-2xl font-bold text-[#ffe08a]">가입할 길드를 선택해주세요</h1>
              <p className="mt-2 text-sm leading-6 text-[#d6b878]">
                {me?.nickname ?? me?.loginId} 계정은 현재 가입된 길드가 없습니다.
                <br />
                길드 가입 요청이 승인되기 전까지는 가입 요청 외 기능을 사용할 수 없습니다.
              </p>
            </div>
            <button
              type="button"
          ...
          <form onSubmit={handleSubmit} className="mt-6 space-y-4">
            {pendingRequest && (
              <div className="rounded-lg border border-[#d5a84a] bg-[#3c2415] px-4 py-3 text-sm text-[#f5e6bd]">
                <div className="font-bold text-[#ffe08a]">가입 요청 대기 중</div>
                <div className="mt-1">
                  {pendingRequest.guildName} 길드에 가입 요청을 보냈습니다. 승인 전에는 다른 길드에 가입 요청할 수 없습니다.
                </div>
              </div>
            )}
