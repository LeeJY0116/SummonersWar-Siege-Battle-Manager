...
전투 연구는 게시글과 댓글이 지속적으로 쌓이는 기능이므로 다음 제한을 둡니다.
...
검색과 필터 조건은 프론트에서 현재 페이지에만 적용하지 않고 백엔드 요청 파라미터로 전달합니다.
백엔드는 전체 게시글을 기준으로 필터링한 뒤 페이지네이션해, 아직 내려받지 않은 페이지의 게시글도 검색 결과에 포함되도록 처리합니다.
